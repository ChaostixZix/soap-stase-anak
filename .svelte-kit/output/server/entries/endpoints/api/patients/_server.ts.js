...
import { g as generateRequestId, a as getUserFromRequest, c as createErrorResponse, b as createServerClient, U as UuidSchema, m as mapSupabaseError, d as createSuccessResponse, i as CreatePatientSchema } from "../../../../chunks/db.js";
...
const POST = async ({ request }) => {
  ...
  try {
    ...
    const { error: bangsalError } = await supabase.from("bangsal").select("id").eq("id", validationResult.data.bangsal_id).eq("hospital_id", validationResult.data.hospital_id).single();
    if (bangsalError) {
      const errorResponse = mapSupabaseError(bangsalError, requestId);
      return json(errorResponse, {
        status: errorResponse.error === "Resource not found" ? createErrorResponse("Invalid bangsal for this hospital", requestId).error === "Invalid bangsal for this hospital" ? 400 : 404 : errorResponse.error === "Access denied" ? 403 : 500
      });
    }
    const { data: _patient, error } = await supabase.from("patient").insert({
      owner_id: user.id,
